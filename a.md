...
| Revisiting Few-Shot Object Detection with Vision-Language Models | NeurIPS (Datasets & Benchmarks) | 2024 | **COCO-FSOD, nuImages-FSOD** (uses LVIS only for pre-training Detic, no LVIS evaluation) | **COCO novel (nAP, 30-shot)**: ≈34.0 for strongest baselines (e.g., DE-VIT / DiGeo, not their own new detector) | **COCO base (bAP, 30-shot)**: ≈29–39 depending on backbone | Focus is on **benchmark design + analysis**, not a single new detector; no LVIS novel/known AP reported, so not directly comparable to your LVIS results; methods are still few-shot supervised (need K labeled examples); pipeline uses Detic / GroundingDINO and federated losses → heavy training and tuning; does not study training-free NOD like your base paper. |
| Few-Shot Object Detection with Foundation Models (FM-FSOD) | CVPR | 2024 | **COCO G-FSOD**, VOC (no LVIS) | **COCO novel (nAP, 30-shot, ViT-L)**: 37.0 | **COCO base (bAP, 30-shot, ViT-L)**: 45.2 | Uses very large backbones (DINOv2 ViT-L) and an LLM-based prompt module; trained for generalized FSOD (G-FSOD) on COCO only; no results on LVIS long-tail split; heavy computation and memory (backbone + LLM + Deformable DETR); not training-free and still needs K-shot labels for novel classes, so not directly comparable to your zero-shot CoFM. |
