...
| Paper / Method | Venue & Year | LVIS Setting | Main Evaluation Measures | Novel-class (rare) performance on LVIS | Key limitations (for your discussion) |
| --- | --- | --- | --- | --- | --- |
| **TFA – Frustratingly Simple Few-Shot Object Detection** (Wang et al.) | ICML 2020 | Treat **rare classes as novel**, frequent+common as base. Fine-tune only the classifier of Faster R-CNN on a balanced subset of LVIS. :contentReference[oaicite:0]{index=0} | LVIS reports **AP, AP50, AP75, APs, APm, APl, APr, APc, APf**. The best LVIS result (R-101, repeated sampling, TFA w/ cosine head) achieves **AP = 26.2**, **APr = 17.3**, **APc = 26.4**, **APf = 29.6**. :contentReference[oaicite:1]{index=1} | **APr ≈ 17.3** for rare (novel) classes, which is clearly higher than the LVIS baseline Joint-training APr (≈13–13.4). :contentReference[oaicite:2]{index=2} | - Still **needs supervised training** on LVIS base + few-shot novel classes (not training-free).  - Uses a relatively simple fine-tune of the last layers; **no language or segmentation priors**, so it may miss very small / unusual novel objects. - Depends on repeated sampling tricks to handle the long-tail; performance drops without that. |
| **Sylph – Hypernetwork Framework for Incremental FSOD** (Yin et al.) | CVPR 2022 | **Incremental few-shot detection (iFSD)** on LVIS: a class-agnostic FCOS base detector is trained, then a hypernetwork generates classification heads for new classes without test-time fine-tuning. :contentReference[oaicite:3]{index=3} | LVIS results in **AP, APr, APc, APf** for different training recipes (Default / Aug / All). In the strongest “All” setting, Sylph achieves **AP = 24.6**, **APr = 16.5**, **APc = 23.7**, **APf = 29.1**. :contentReference[oaicite:4]{index=4} | **APr ≈ 16.5** on rare classes in the “All” setup – competitive with TFA, but slightly lower rare AP while being finetune-free at test time. :contentReference[oaicite:5]{index=5} | - Needs a **large, fully-supervised base detector** trained on many LVIS classes; performance depends on that heavy pretraining. - Hypernetwork mainly improves **classification for novel classes**, but localization quality is inherited from the base FCOS and can still be weak on very small or occluded rare objects. - Still relies only on visual features; **no VLM / text guidance** or open-vocabulary capability. |
| **iTFA – Incremental Few-Shot Object Detection via Simple Fine-Tuning Approach** (Cho et al.) | ICRA 2023 | **Incremental FSOD (iFSD)** on LVIS with a Faster R-CNN base detector. Base training uses common+frequent classes; fine-tuning only uses few-shot rare classes, with separate branches for base vs novel. :contentReference[oaicite:6]{index=6} | On LVIS v1, they report **AP, APs, APm, APl, APr, APc, APf**. In Table III, iTFA (“Ours”) gets **AP = 24.0**, **APr = 18.1**, **APc = 21.0**, **APf = 29.9** – improving rare AP over TFA and Sylph under their iFSD protocol. :contentReference[oaicite:7]{index=7} | **APr ≈ 18.1** on rare classes (treated as novel classes), which is slightly better than TFA (17.3) and Sylph (16.5) under the reported setting. :contentReference[oaicite:8]{index=8} | - Still a **supervised two-stage pipeline**; needs labeled LVIS base and novel data and fine-tuning per incremental step. - Requires careful layer freezing and separate branches; more complex to implement and slower than purely training-free methods like your base paper. - Like the others, **no VLM or segmentation prior**, so it may struggle with unseen categories outside the LVIS label space. |
