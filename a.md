...
| **UIFormer: A Unified Transformer-based Framework for Incremental Few-Shot Object Detection and Instance Segmentation** | – (arXiv preprint) | 2024 | COCO, **LVIS v1** (iFSOD + iFSIS). LVIS rare = novel; common+frequent = base. :contentReference[oaicite:0]{index=0} | **LVIS (iFSOD, 10-shot):** 22.46 AP on *novel/rare* classes. :contentReference[oaicite:1]{index=1} | **LVIS base:** 16.98 AP on common + 32.34 AP on frequent classes (~24.7 AP on average). :contentReference[oaicite:2]{index=2} | • **Incremental few-shot (not training-free):** needs base-stage training + novel-stage fine-tuning, so it does not match our *zero-shot* NOD setting.<br>• **Transformer heavy & segmentation-oriented:** built on Mask-DINO with joint detection + segmentation; higher memory/compute than our CLIP+GDINO+SAM pipeline.<br>• **Complex training recipe:** multi-stage pre-training, fine-tuning, knowledge-distillation; hard to reproduce and integrate into a modular NOD framework like ours.<br>• **LVIS results reported with AP (not mAP@[0.5:0.95] under known/novel split used in CoFM):** cannot be directly compared to our training-free LVIS NOD numbers. |
| **Expandable-RCNN: Toward High-Efficiency Incremental Few-Shot Object Detection** | *Frontiers in Artificial Intelligence* | 2024 | COCO, **LVIS** (incremental FSOD); LVIS rare treated as novel; frequent/common as base. :contentReference[oaicite:3]{index=3} | – (authors state improvements on LVIS rare classes but exact AP for rare classes is only in the closed PDF). | – (evaluated on LVIS base classes; exact AP for common/frequent classes not visible in public abstract). | • **Faster-RCNN based with weight imprinting:** still a conventional detector; no use of VLM semantic priors (CLIP/GDINO), so generalization to unseen text categories is weaker than our base paper.<br>• **Requires sequential retraining steps:** although “zero retraining” is claimed for the *backbone*, the classifier needs weight-imprinting + group softmax tuning; still a supervised pipeline, unlike our training-free CoFM design.<br>• **Evaluation is iFSOD on LVIS, not LVIS-NOD:** focuses on adding a few labeled instances for new LVIS classes, not on fully zero-shot novel discovery from prompts like our method.<br>• **LVIS metrics not fully reported in open sources:** hard to make a precise, fair numerical comparison in our literature table; we can only describe relative gains over ONCE and other baselines. |
